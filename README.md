...
- MODE: Attention here! This is the mode number that you want to be associated with the route and to be used in the final .lin file. You can create whatever mode number you want to categorize the routes accordingly. There is an input variable named 'mode' in the main script but it is related to the mode numbers in GTFS (See below 'Running the script').
...
- The third process in the Cube app produces the final '.net' that will be stored as 'NETWORK.NET' in './BASE/YOUR_SCENARIO_FOLDER/network'.
...
- The line file containing line basic information and sequence of nodes (stop or non-stop nodes). The file is named as 'PTlines.lin'. The current line information included are the following. They are hard-coded, but in future we can make it more flexible to read the wanted information from "./inputs/YOUR_PROJECT_FOLDER/route-info/modes_table.csv" or other idea.
  ...
  - HEADWAY: As many as items found in the dictionary variable 'period_times' (check the input variables in 'Running the script' section). If two items are given to 'period_times', e.g., 'AM' and 'PM' periods, two HEADWAY information will be found in 'PTlines.lin' as: HEADWAY[1]=X, HEADWAY[2]=Y. X and Y are the calculated headway for each period.
...
However, make sure you install an Anaconda version that comes with Python 3.9.X. We recommend you to install Anaconda 2022.10 (<link>[here](https://repo.anaconda.com/archive/Anaconda3-2022.10-Windows-x86_64.exe)</link>).
...
- Open the terminal from the project's folder
  ...
  - Or you can run cmd (Command Prompt) and change the base directory to project's folder by entering <i>cd PATH\TO\PROJECT</i>.
- In the terminal, enter 
  - <i>python -m venv gtfs2ptnet_venv</i> to create the venv named as <i>gtfs2ptnet_venv</i>. A folder with this name will be created in the project's folder.
  - then, <i>gtfs2ptnet_venv\Scripts\activate</i> to activate the venv.
  - then, <i>pip install -r requirements.txt</i> to install all the necessary packages found in <i>requirements.txt</i> to your venv.

## Running the tool

You have two options to run this tool:
- Open the Cube catalog file "gtfs2net.cat" and create a scenario with your inputs (<i>preferred!</i>).
- Or run the Python script directly from the terminal by doing the following
  ...
  - Open the terminal from the project's folder (check how in section "Creating a virtual environment and installing requirements.txt").
  - Enter <i>gtfs2ptnet_venv\Scripts\activate</i> to activade the venv.
  - Finally, enter <i>python gtfs2ptnet_main.py "FULL/PATH/TO/parameters.txt"</i> to run the script with the specified parameters.txt (use the full path to this file).
...
- net_proj: Projection as EPSG or ESRI code (find them <a href='https://spatialreference.org/'>here</a>) of your network. For buffers in feet, use a projection that has feet as standard unit.
  - At MORPC, we use the projection 'NAD 1983 StatePlane Ohio South FIPS 3402 Feet' (<a href='https://spatialreference.org/ref/esri/102723/'>here</a>). In our case, we use: net_proj = "ESRI:102723"
